...
def dat_tiny_patch16_224(num_classes=1000, drop_path_rate=0., **kwargs):
    return DAT(
        img_size=224, patch_size=4, num_classes=num_classes, expansion=4, dim_stem=96, dims=[96, 192, 384, 768],
        depths=[2, 2, 6, 2], stage_spec=[['L', 'S'], ['L', 'S'], ['L', 'D'] * 3, ['L', 'D']],
        groups=[-1, -1, 3, 6], use_pes=[False, False, True, True], dwc_pes=[False, False, False, False],
        strides=[-1, -1, 1, 1], offset_range_factor=[-1, -1, 2, 2], drop_path_rate=drop_path_rate, **kwargs
    )


def dat_small_patch16_224(num_classes=1000, drop_path_rate=0., **kwargs):
    return DAT(
        img_size=224, patch_size=4, num_classes=num_classes, expansion=4, dim_stem=96, dims=[96, 192, 384, 768],
        depths=[2, 2, 18, 2], stage_spec=[['L', 'S'], ['L', 'S'], ['L', 'D'] * 9, ['L', 'D']],
        groups=[-1, -1, 3, 6], use_pes=[False, False, True, True], dwc_pes=[False, False, False, False],
        strides=[-1, -1, 1, 1], offset_range_factor=[-1, -1, 2, 2], drop_path_rate=drop_path_rate, **kwargs
    )


def dat_base_patch16_224(num_classes=1000, drop_path_rate=0., **kwargs):
    return DAT(
        img_size=224, patch_size=4, num_classes=num_classes, expansion=4, dim_stem=128, dims=[128, 256, 512, 1024],
        depths=[2, 2, 18, 2], stage_spec=[['L', 'S'], ['L', 'S'], ['L', 'D'] * 9, ['L', 'D']], heads=[4, 8, 16, 32],
        groups=[-1, -1, 4, 8], use_pes=[False, False, True, True], dwc_pes=[False, False, False, False],
        strides=[-1, -1, 1, 1], offset_range_factor=[-1, -1, 2, 2], drop_path_rate=drop_path_rate, **kwargs
    )
